...
async function main() {
  ...
  if (args.build) {
    ...
    console.log("");
    const buildResult = spawnShell("build", {
      ...process.env,
  ...
  const oldBlobIds =
    typeof gcResult === "object" && gcResult.blobIds ? gcResult.blobIds : [];
  if (deletedCount > 0) {
    console.log(
  ...
  if (oldBlobIds.length > 0) {
    const cdnDeleteFn =
      args.cdnDeleteFunction || `${componentName}:deleteCdnBlobs`;
    try {
      ...
      console.warn(
        `Warning: Could not delete old CDN blobs. Make sure ${cdnDeleteFn} is defined.`,
      );
    }
  }
  console.log("");
